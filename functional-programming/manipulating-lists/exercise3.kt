...
fun authorBooksMap(books: List<Book>): Map<Author, List<Book>> {
  val authors = books.flatMap { it.authors }.toSet() // get set of authors
  return authors.map { author -> author to books.filter { author in it.authors } }.toMap() // return map of books and authors
}
